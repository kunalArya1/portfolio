      ...
      <div className="p-4 flex flex-col h-[calc(100%-13rem)]">
        ...
        <div className="flex items-center justify-between mt-auto">
          <div className="flex items-center space-x-2.5">
            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-purple-500/10 to-blue-500/10 flex items-center justify-center">
...
const BlogPage: React.FC<BlogPageProps> = ({ allPosts }) => {
  ...
  useEffect(() => {
    ...
    console.log('Featured posts:', allPosts.filter(post => post.Featured));
    ...
    const filtered = allPosts.filter((blog: Post) => {
      ...
      return matchesSearch && matchesTags && matchesAuthor && matchesDate && matchesFeatured;
    });

    console.log('Filtered blogs:', filtered);
    setFilteredBlogs(filtered);
  }, [allPosts, search, selectedTags, selectedAuthor, selectedDate, showFeaturedOnly]);
        ...
        <div className="relative max-w-7xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6 mb-8">
            <div className="max-w-xl">
              ...
              <p className="text-gray-400 mt-2">
                Deep dives into web development, coding best practices, and tech innovations
              </p>
            </div>
          ...
          <div className="flex flex-col-reverse md:flex-row gap-6 justify-between">
            ...
            <div className="flex flex-wrap gap-4">
              <div className="px-4 py-2 rounded-xl border border-[#2a2a2a] flex items-center gap-3 cursor-pointer hover:bg-[#2a2a2a] transition-colors" onClick={() => setShowFeaturedOnly("no")}>
                ...
                <div>
                  <div className="text-lg font-semibold text-white">{totalBlogs}</div>
                  <div className="text-xs text-gray-400">Total Articles</div>
                </div>
              </div>

              <div className="px-4 py-2 rounded-xl border border-[#2a2a2a] flex items-center gap-3 cursor-pointer hover:bg-[#2a2a2a] transition-colors" onClick={() => setShowFeaturedOnly(prev => prev === "no" ? "yes" : "no")}>
                <div className="w-8 h-8 rounded-lg flex items-center justify-center">
                  <FaStar className={`w-4 h-4 ${showFeaturedOnly ? 'text-yellow-400' : 'text-pink-400'}`} />
                </div>
                <div>
                  <div className="text-lg font-semibold text-white">{featuredBlogs}</div>
              ...
              <div className="px-4 py-2 rounded-xl border border-[#2a2a2a] flex items-center gap-3">
                ...
                <div>
                  <div className="text-lg font-semibold text-white">{uniqueTags}</div>
      ...
      <div className="max-w-7xl mx-auto pb-12">
        {filteredBlogs.length > 0 ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-gray-400">Try adjusting your search criteria or explore different tags</p>
          </div>
        )}
